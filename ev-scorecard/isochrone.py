...
for nearby public charging when home charging isn't available (see
charger_access.py). Walk-only; unlike city-scorecard, EV access doesn't need
a bike isochrone.
"""
...
def compute_isochrone(G, lat: float, lon: float) -> IsochroneResult:
    ...
    for _, _, data in G.edges(data=True):
        data["travel_time"] = data["length"] / (WALK_SPEED_KMH * 1000 / 3600)
